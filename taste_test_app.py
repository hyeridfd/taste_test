...
def insert_taste_response(response_data: dict):
    ...
    sb.table("taste_test_responses").insert(row).execute()
...
def fetch_taste_responses_df() -> pd.DataFrame:
    ...
    if sb is None:
        return pd.DataFrame()
    res = sb.table("taste_test_responses").select("*").order("제출시간", desc=True).execute()
...
def main():
    ...
    with st.sidebar:
        st.markdown("---")
        admin_mode = st.checkbox("🔧 관리자 모드", value=False)
    ...
    if admin_mode:
        ...
        if not df_db.empty:
            ...
            # 표시할 컬럼 선택
            display_cols = ["성명", "이메일", "성별", "나이", "제출시간"]
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"미각테스트_전체응답_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv",
                use_container_width=True
            )
